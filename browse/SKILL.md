...
```bash
# Check the user-level Codex install
if test -x ~/.codex/skills/browse/dist/browse; then
  echo "READY"
else
  echo "NEEDS_SETUP"
fi
...
Set `B=~/.codex/skills/browse/dist/browse` and use it for all commands.

If `NEEDS_SETUP`:
1. Run the one-time build in the skill directory:
```bash
cd ~/.codex/skills/browse && ./setup
```
2. If `bun` is not installed, tell the user to install it: `curl -fsSL https://bun.sh/install | bash`

Once setup is done, it never needs to run again (the compiled binary persists).

## IMPORTANT

- Use the compiled binary via Bash: `~/.codex/skills/browse/dist/browse`.
...
```bash
B=~/.codex/skills/browse/dist/browse
